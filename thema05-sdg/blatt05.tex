...
Dritten.
\item Jede glatte Funktion\footnotemark{} ist in~$\U$ enthalten.
...
Finde zu jeder der folgenden Funktionen~$f$ die vom Lemma von Hadamard
versprochene Funktion~$g$, also die Funktion~$g$ mit~$f(x) = f(0) + x \cdot
g(x)$ für alle reellen Zahlen~$x$.
\begin{enumerate}
\item $f(x) = 3 + 2x$.
\item $f(x) = x^2$.
\item $f(x) = x^2 - x$.
