...
denen wir zwei weiter unten skizzieren werden.
...
keine widersprüchlichen Aussagen folgen. Dabei werden wir auch die Verbindung
zur klassischen Mathematik klären.
...
erfordert jedoch viele Vorkenntnisse aus der \emph{Topostheorie}, die
wir hier leider nicht bereitstellen können. Die Konstruktion eines solchen Modells
können wir daher an dieser Stelle nicht durchführen, folgenden Fakt müssen wir
hinnehmen:

\begin{shaded}
\textbf{Fakt.} Es gibt ein mathematisches Universum~$\U$ mit folgenden
Eigenschaften:
\begin{itemize}
...
oft} differenzierbar ist, wenn also nicht nur~$f$ selbst differenzierbar ist,
sondern auch~$f'$,~$f''$ und so weiter.

\begin{aufgabeShaded}{Eine nicht glatte Funktion}
Mache dir klar, dass folgende Funktion~$f : \RR \to \RR$ zwar differenzierbar,
...
\[ f(x) = f(0) + x \cdot g(x). \]
\end{lemma}
\begin{proof}Diese Aussage ist in der Literatur als \emph{Lemma von Hadamard}
bekannt. Mit Tipps beweisen wir sie in einer Aufgabe.\end{proof}

\begin{aufgabeShaded}{Beispiele zum Lemma von Hadamard}
Finde durch geeignete Termumformungen zu jeder der folgenden Funktionen~$f$ die
vom Lemma von Hadamard versprochene Funktion~$g$, also die Funktion~$g$
mit~$f(x) = f(0) + x \cdot g(x)$ für alle reellen Zahlen~$x$.
\begin{enumerate}
\item $f(x) = 3 + 2x$.
\item $f(x) = x^2$.
\item $f(x) = x^2 - x$.
...
\begin{aufgabeShaded}{Bedeutung des Lemmas von Hadamard}
Sei~$f : \RR \to \RR$ eine glatte Funktion und~$g$ die laut dem Lemma von
Hadamard existente Funktion mit~$f(x) = f(0) + x \cdot
g(x)$ für alle reellen Zahlen~$x$. Zeige: $g(0) = f'(0)$.

\emph{Tipp:} Leite die gegebene Gleichung, die die Beziehung zwischen~$f$
und~$g$ beschreibt, ab.
\end{aufgabeShaded}

\begin{prop}Sei~$f : \RR \to \RR$ eine glatte Funktion. Gelte laut~$\U$,
dass~$f'(0) = a$. Dann gilt auch wirklich~$f'(0) = a$.
\end{prop}
