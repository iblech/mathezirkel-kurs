...
\begin{frame}{Choice functions}
  ...
  {\centering\emph{\scriptsize The axiom of choice asserts:}\\[-0.5em]\begin{minipage}{0.62\textwidth}\begin{block}{}
    \centering
  ...
  \begin{enumerate}
    ...
    \item squaring function: $x \mapsto x^2$, so $1 \mapsto 2,\ 2 \mapsto 4,\
    3 \mapsto 9,\ \ldots$
...
\begin{frame}{Beyond the axiom of choice}
  \begin{enumerate}
    \item There is an alternate opposing axiom, the \hil{axiom of determinacy}: \\
    ``Every instance of the \hil{sequence game} is \hil{determined}.''
    \bigskip
    \pause

    \item Introducing the axiom of choice does not yield new inconsistencies.
    \bigskip
    \pause

    \item Even if the axiom of choice fails, it always holds in~$L$, \hil{Gödel's sandbox}.
    \pause
    \justifying
    \medskip

    Since~\textsf{Std}'s and~$L$'s $\NN$ coincide, \textsf{Std} and
    $L$ share the same \hil{arithmetic truths}.
    \medskip

    From every proof of such a truth using the axiom of choice, the axiom of
    choice can be \hil{mechanically eliminated}.
    \bigskip
    \pause

    \item Much more severe than the axiom of choice is the \hil{powerset axiom}.
