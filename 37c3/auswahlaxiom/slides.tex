...
\begin{frame}[c]
  ...
  \jnote{3-}{
    ...
    Surprisingly, despite appearances, there \emph{is} a suitable strategy---%
...
\begin{frame}{Choice functions}
  \jnote{1-}{
    The slides uses the terms ``collection'' and ``set'' interchangeably. A set
  ...
  \jnote{4-}{
    ...
    In case~B, we can write down a choice function by picking the smallest
...
\begin{frame}{Consequences of the axiom of choice}
  \jnote{1}{
    ...
    philosophically connect with its statement, but also explore its
    consequences. One reason that the axiom of choice is somehow contested is
    that it entails both consequences which are commonly considered ``bad'' and
    ...
    \begin{enumerate}
      ...
      \item A solid three-dimensional ball can be disassembled into five number
      of pieces in such a way that these pieces can be reassembled (after
  ...
  \jnote{2-}{
    ...
    \begin{enumerate}\justifying
      ...
      universe. \emph{Some results otherwise obtained using the axiom of choice can be
...
\begin{frame}{A counterexample to the axiom of choice}
  ...
  identical programs} would look like this:
  \[
    \small
    \begin{array}{lll}
      \left\{
  ...
  With such a choice function~\textsf{c}, a \hil{halting oracle} can be built: \\
...
\begin{frame}{A varied perspective on the axiom of choice}
  \jnote{2-}{
    The standard foundational system for mathematics commonly accepted by
    logicians is~\textsc{zfc}, Zermelo--Fraenkel set theory with the axiom of
    choice. \textsc{zf} is the variant without~\textsc{ac}. \textsc{pra} is a
    certain base theory so weak that it is contested just by ultrafinitists and
    hence often used for metamathematical pursuits.
  }
