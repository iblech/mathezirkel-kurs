...
\title[Alternate mathematical universes]{Alternate mathematical universes}
\author[Ingo Blechschmidt]{\textcolor{white}{Ingo Blechschmidt}}
\date[2016-12-28]{\vspace*{-7em}\ \\\textcolor{white}{\scriptsize Institut für Mathematik \\ Universität Augsburg \\ December 28th, 2016}}
...
\begin{frame}{On truth}
  ...
  \begin{itemize}
    ...
    \item The usual axioms of logic hold in the \hil{standard topos}~$\Set$.
    \item In other topoi, slightly different axioms are true.
  \end{itemize}
\end{frame}


\subsection{Examples for topoi}

\begin{frame}{Examples}
  There are topoi in which \ldots
  \begin{itemize}
    \item there is an intermediate infinity between~$\NN$ and~$\RR$,
    \item there is no such intermediate infinity,
    \item there are \hil{infinitesimal real
    numbers}~$\varepsilon$ such that~$\varepsilon \neq 0$ but $\varepsilon^2 = 0$, or
    \item any function $\RR \to \RR$ is a polynomial.
  \end{itemize}
  \bigskip
  \pause

  \begin{center}
    \fmini{0.19\textwidth}{proving}
    {\usebeamercolor[fg]{item}$\boldsymbol{\xleftrightarrow{\quad\text{\normalsize effective topos}\quad}}$}
    \fmini{0.27\textwidth}{programming}
  \end{center}
\end{frame}


\subsection{The effective topos}
...
\begin{frame}{The effective topos}
  ...
  \only<1>{\begin{center}
    \includegraphics[width=0.4\textwidth]{images/turing-machine}
  \end{center}}
...
\begin{frame}{Our first steps in the effective topos}
  ...
  prime.''}$}{There is a program which reads a number $n$ as input and outputs
...
\subsection{What's true in alternate topoi?}
...
\subsection[Praise]{Appreciating constructive logic}
...
\subsection[Functions]{Equality of functions}
...
\begin{frame}{LEM for equality of functions}
  ...
  not.''\\[-0.7em]\end{minipage}$}{There is a program which reads the source of a program~$M$, which calculates a function $\NN \to \NN$, as input, and finds out whether~$M$ always yields zero or not.}{That's false.}
  \bigskip
  \pause
  ...
  \begin{center}
    \includegraphics[width=0.3\textwidth]{images/ordinal-omega-squared}
  \end{center}
\end{frame}


\subsection[Halting]{The halting problem}
...
\subsection[Reals]{Equality of real numbers}
...
\begin{frame}{LEM for equality of real numbers}
  ...
  decimal digit is a one if and only if~$M$ halts after step~$n$.
  \bigskip
...
\subsection[Markov]{Markov's principle}
...
\begin{frame}{Markov's principle}
  ...
  0$.''\\[-0.7em]\end{minipage}$}{There is a program which reads the source of a
  program~$M$, which calculates a function~$\NN \to \NN$ which is not the zero
  function, as input and outputs a number~$n$ such that~$M(n)$ is not
  zero.}{\pause That's true! By unbounded search.}
\end{frame}


\subsection[Imp.\@]{Searching uncountable sets}
...
\subsection[Church--Turing]{The Church--Turing thesis}
...
\begin{frame}{The Church--Turing thesis}
  ...
  program.''\\[-0.7em]\end{minipage}$}{There is a program which reads the source of a
  ...
  In~$\Eff(\STM)$ and~$\Eff(\lambdaC)$ the statement is false.
\end{frame}
...
\subsection[Cont.\@]{Automatic continuity}
...
\begin{frame}{Wrapping up}
  \begin{itemize}
    \item Effective topoi are a good vehicle for studying the nature of computation.
    \item The effective topoi build links between constructive mathematics and programming.
    \item Topoi allow for curious dream axioms.
    \item Topoi have a geometric flavour: points, subtopoi, maps between topoi.
  \end{itemize}
  \pause

  \centering
  \bigskip
  \hil{There is more to mathematics \\
