...
\date[2017-09-07]{\vspace*{-4em}\ \\\textcolor{white}{\scriptsize Curry Club Augsburg \\ 7. September 2017 und 5. Oktober 2017}}
...
\begin{frame}{Peano-Arithmetik}
  Die Schlussregeln und Axiome von~$\PA$ sind:
  \begin{itemize}
    \item die üblichen Schlussregeln von Logik erster Ordnung
    \item $\neg (\exists n\_ 0 = S(n))$
    \item $\forall n\_ \forall m\_ (S(n) = S(m) \rightarrow n = m)$
    \item $\forall n\_ n + 0 = n$ \\
    \item $\forall n\_ \forall m\_ n + S(m) = S(n + m)$ \\
    \item $\forall n\_ n \cdot 0 = 0$ \\
    \item $\forall n\_ \forall m\_ n \cdot S(m) = n \cdot m + n$
...
\begin{frame}{Undefinierbarkeit von Wahrheit}
  \begin{block}{Erinnerung: Reflektion von Beweisbarkeit}
    Es gibt eine Aussageform~\hil{$\boldsymbol{\Prov(n)}$}, sodass für jede Aussage~$A$
    genau dann~$\Prov(\goedel{A})$ wahr ist, wenn~$A$ \hil{beweisbar} ist:
    \vspace*{-0.8em}
    \[ \NN \models \Prov(\goedel{A}) \quad\text{genau dann, wenn}\quad
      \PA \proves A. \]
  \end{block}
...
\begin{frame}{Unentscheidbarkeit des Halteproblems}
  Ein \hil{Halteorakel} ist ein Programm, das ein Programm~$P$ als Eingabe liest
...
\begin{frame}{Chaitinsche Haltewahrscheinlichkeit}
  Sei~$\Omega$ die Zahl
  \[ \Omega = \sum_p 2^{-|p|} = c_0 \cdot 1 + c_1 \cdot \frac{1}{2} + c_2 \cdot
  \frac{1}{4} + c_3 \cdot \frac{1}{8} + \cdots\!, \]
...
\begin{frame}{Das universelle Programm}
  ...
  \begin{enumerate}
    ...
    (vielleicht die leere Liste, falls~$P$ nicht halten sollte).
    \item\justifying Für jede endliche Liste von Zahlen gibt es ein
    Universum~$U$, sodass~$P$ genau diese Liste ausgibt, wenn man es in~$U$
    ausführt.
  \end{enumerate}
\end{frame}
...
\begin{frame}{Ein Rosetta-Stein}
  ...
  Eine der Konsequenzen: Es gibt eine diophantische Gleichung, die genau dann
  eine Lösung hat, wenn~$\PA$ inkonsistent ist.
