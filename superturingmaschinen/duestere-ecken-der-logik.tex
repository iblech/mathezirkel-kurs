...
\note{\justifying\scriptsize
  ...
  Oft verwendet man aber auch starke Systeme wie Zermelo--Fraenkel mit
...
\note{\justifying\scriptsize
  ...
  System von Logik erster Ordnung. Genauso gut könnten wir eine Formalisierung
...
\note{\justifying\scriptsize
  ...
  \begin{itemize}
    \item die üblichen Schlussregeln von Logik erster Ordnung
...
\note{\justifying\scriptsize
  ...
  In~$\NN$ gelten die Peano-Axiome, und unsere Metaebene macht die Schlussregeln
  von Peano-Arithmetik mit. Deswegen sind beweisbare Aussagen auch wahr.
  Überdies sind beweisbare Aussagen auch in allen Nichtstandardmodellen war.
  \medskip
  ...
  \href{https://boolesrings.org/victoriagitman/2015/04/22/an-introduction-to-nonstandard-models-of-arithmetic/
  }{Blog von Victoria Gitman}.
  \par
}

\note{\justifying\scriptsize
  Mit unseren Vereinbarungen an die Metaebene ist~$\PA$ \emph{konsistent},
  d.\,h. die Aussage~$\bot$ (oder äquivalent die Aussage~$0 = 1$) ist nicht
  beweisbar. Denn wenn~$0 = 1$ beweisbar wäre, so wäre auch~$0 = 1$ im
  Standardmodell~$\NN$; dem ist aber nicht so.
  \medskip
...
\note{\justifying\scriptsize
  Die mit dem Diagonallemma konstruierte Aussage~$A$ drückt umgangssprachlich
  aus: "`Aussage~$A$ ist nicht beweisbar."' Der auf der vorhergehenden Folie
