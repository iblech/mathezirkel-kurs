\documentclass[12pt,compress,ngerman,utf8,t,aspectratio=169]{beamer}
...
\begin{frame}{An outline of \ZFC}
  ...
  \textbf{Ex.} $\ZFC \vdash 2+2=4$, \quad $\ZFC \vdash 2+2\neq5$, \quad
  $\ZFC \not\vdash 2+2=5$.
\end{frame}
...
\begin{frame}{Foundational incompleteness}
  ...
  \begin{hilblock}\justifying
    ...
    Then there are mathematical statements~$\varphi$ such that:
    \begin{itemize}
      \item If~$S \vdash \varphi$, then~$S$ is inconsistent.
      \item If~$S \vdash \neg\varphi$, then~$S$ is inconsistent.
    \end{itemize}
  \end{hilblock}
  \medskip
  \pause
...
\begin{frame}{The continuum hypothesis and \\ its traditional dream solution}
  \includegraphics[width=0.7\textwidth]{cardinal-number-line}

  \only<2>{
    \centering
    \bigskip
    \begin{tabular}{ccc}
      \underline{\quad all natural numbers \quad} && \underline{\quad all integers \quad} \\
      $0$ & \rr & $\phantom{+}0$ \\
      $1$ & \rr & $\phantom{+}1$ \\
      $2$ & \rr & $-1$ \\
      $3$ & \rr & $\phantom{+}2$ \\
...
\begin{frame}{Traveling the multiverse}
  ...
  \only<1>{\bigskip\bigskip\centering\includegraphics[width=0.4\textwidth]{multiverse}}
  \pause
...
{\usebackgroundtemplate{\begin{minipage}{\paperwidth}\centering\includegraphics[width=\paperwidth]{geology-light}\end{minipage}}
...
% Explain the platonic heaven more carefully.
