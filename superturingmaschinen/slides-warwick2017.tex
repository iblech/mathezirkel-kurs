...
\note{\begin{itemize}
  \item\justifying There is also a deep link between computability and number
  theory: A set~$M \subseteq \NN$ is enumerable by a Turing machine if and only
  if it is a \hil{diophantic set}, that is of the form
...
\note{\justifying
  For any model~$\mathcal{M}$ of computation, such as Turing machines, super
  Turing machines, or lambda calculus, there is an associated \hil{ef{}fective
  topos} $\mathrm{Eff}(\mathcal{M})$. Formally, a topos is a certain kind of
  category; intuitively, a topos is an alternate mathematical universe in which
  the usual laws of logic may not necessarily hold.
  \bigskip
  ...
  ``model''~$\mathcal{M}$ used for constructing the ef{}fective topos.
...
\note{\justifying
  ...
  This constitutes a constructively valid argument, since in constructive (and
...
\note{\justifying
  ...
  witness of the existential statement (the position where the key is at).
  \bigskip
...
\note{\justifying
  ...
  be a ``yes''. Tricks using black holes and relativistic time dilation might
  be possible.
  \par
}
...
\note{\justifying
  The meaning of the statement in~$\Eff(\STM)$ is: ``There is a super Turing
  machine which reads the source of a super Turing machine computing a
  function~$f : \NN \to \NN$ as input and outputs the source of an
  \emph{ordinary} Turing machine which computes~$f$.'' This is false.
  \bigskip

  The statement is also false in~$\Eff(\lambdaC)$, the effective topos
  associated to lambda calculus.
  \par
}

\note{\justifying
  The topos~$\Eff(\TM)$ is a nice context for computer science, since in it
  every uncomputable functions are not present.\bigskip
  ...
  \end{align*}
  Can't we use exactly the same definitions in the effective topos, thereby
  contradicting the theorem that all functions in the effective topos are
  computable?\par
}
...
\begin{frame}{Curious size phenomena}
  \only<1-2>{
    ...
    %{\centering
    %  \includegraphics[width=0.8\textwidth]{groessenverhaeltnisse}
    %  \par}

    \pause
    In~$\Eff(\STM)$, there is such an injection.
  }
