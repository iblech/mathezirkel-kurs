...
\note{\justifying
  ...
  \[ L(\BB) = \prod_{n=0}^\infty \BB = \{ (x_0,x_1,\ldots) \} \]
  can. There is a deeper topological reason for this fact: The space~$L(\BB)$
  is compact while~$\NN$ is not.
  \bigskip
...
\note{\justifying\scriptsize
  ...
  the numbers~$f(x)$ and~$f(\tilde x)$ are the same to~$n$ digits.\bigskip
  ...
  to react to~$X'$ in the same way as it reacts to~$X$.\bigskip

  If only finitely many computational steps can be performed in finite time,
  the machine~$M$ only has to wait till~$A$ has a computed~$f(x)$ to~$n$
  digits. It can then look at its log to see how many digits of~$x$ $A$ used
  for the computation. For any number~$\tilde x$, which is equal to~$x$ to this
  amount of digits, the machine~$A$ produces the same output.\par
}
