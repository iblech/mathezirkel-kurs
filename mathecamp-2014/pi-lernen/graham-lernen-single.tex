\documentclass{zirkelblatt-single}

\usepackage{geometry,wrapfig}
\geometry{paperwidth=89mm,paperheight=59mm,tmargin=0.2cm,bmargin=0.5cm,lmargin=0cm,rmargin=0cm}
...
\AddToHook{shipout/background}{%
  \put(0in,-\paperheight){\includegraphics[width=\paperwidth]{sun3-lighter}}%
}
...
\begin{minipage}[t][4cm][t]{7.6cm}
  \begin{center}
    \ \\[-0.9em]
    \textbf{Graham's number} \\[0.3em]
    \scriptsize
    {an enormous number in Ramsey theory \\ whose first digit might
    remain unknown for all time \\ but whose last 500 digits can be computed and are shown below:}
    \vspace{0.6em}
...
\AddToHook{shipout/background}{%
  \put(0in,-\paperheight){\includegraphics[width=\paperwidth]{sun3}}%
}
\begin{minipage}[t][4cm][t]{7.6cm}
  \scriptsize
  ...
  and the same holds for the four-, five-, \ldots, and 13-dimensional cube.
  \medskip
